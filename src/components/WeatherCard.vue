...
const timeStr = computed(() =>
  props.time.toLocaleTimeString("de-DE", {
    hour: "2-digit",
    minute: "2-digit",
  }),
);
...
<template>
  <div class="flex flex-col p-10 gap-y-10">
    ...
    <div
      v-if="error"
      class="py-10 flex items-center justify-center text-red-400 text-xl"
    >
    ...
    <template v-else-if="isEvening && tomorrow !== null">
      <div class="grid grid-cols-3 gap-4">
        ...
        >
          <span
            class="text-xl text-neutral-500 dark:text-neutral-400 font-mono"
            >{{ formatHour(slot.hour) }}</span
          >
          <span class="text-6xl leading-none">{{
            getWeatherCondition(slot.weatherCode).icon
          }}</span>
          <span class="text-5xl font-bold leading-none"
            >{{ slot.temperature }}°</span
          >
          <span
            class="text-xl text-neutral-600 dark:text-neutral-300 text-center leading-tight"
          >
          ...
          <span
            ...
            :class="{
              ...
              'text-amber-400':
                slot.precipitationProbability > 20 &&
                slot.precipitationProbability <= 50,
      ...
      <div class="flex justify-between items-center text-2xl">
        <span class="text-neutral-600 dark:text-neutral-300"
          >↑ {{ tomorrow.high }}°&nbsp;&nbsp;↓ {{ tomorrow.low }}°</span
        >
        <span
          class="text-lg text-neutral-500 font-medium tracking-widest uppercase"
          >Morgen</span
        >
